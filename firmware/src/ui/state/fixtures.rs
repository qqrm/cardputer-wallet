...
pub fn generate_entries(config: EntryFixtureConfig) -> Vec<EntrySummary> {
    const TITLES: [&str; 10] = [
...
impl SystemAdapter {
    ...
    pub fn with_unlock_status(mut self, status: PinLockStatus) -> Self {
        self.unlock_status = status;
        self
    }

    pub fn with_unlock_error(mut self, error: PinUnlockError) -> Self {
...
pub fn submit_pin(ui: &mut UiRuntime, adapter: &SystemAdapter, pin: &str) {
